...
function renderProduct(){
  ...
  if (mainImage) {
    mainImage.src = "../../" + firstImg || "";         // si vide -> src vide (tu peux styler)
  ...
  if (whatsappLink) {
    const msg = `Bonjour, je souhaite commander: ${product.title} (id:${product.id}) - quantité: `;
    whatsappLink.href = `https://wa.me/24177067949?text=${encodeURIComponent(msg)}`; // <- remplace par ton n°
  }
}
...
/* -------------------------
   Similar products (utilise uniquement images depuis la BD)
   ------------------------- */
function renderSimilar(){
  if (!similarGrid) return;
  similarGrid.innerHTML = "";
  const sims = PRODUCTS.filter(p => p.id !== product.id && p.category === product.category).slice(0,4);
  if (!sims.length) {
...
function attachUiHandlers(){
  if (qtyMinus) qtyMinus.addEventListener("click", ()=> { qtyInput.value = Math.max(1, Number(qtyInput.value||1) - 1); });
  if (qtyPlus)  qtyPlus.addEventListener("click", ()=> { qtyInput.value = Math.max(1, Number(qtyInput.value||1) + 1); });
